function Con_2_N = red_density_matrix_2_N(Psi,N) 
...
Con_2_N = max(0,sqrt(EV(1))-sqrt(EV(2))-sqrt(EV(3))-sqrt(EV(4)));
% 
% R = sqrtm(sqrtm(Rho_r_2_N)*rho_tilt*sqrtm(Rho_r_2_N));
% EV_R = sort(eig(R),'descend');
% Con_2_N  = (EV_R(1)-EV_R(2)-EV_R(3)-EV_R(4));

Con_2_N = real(Con_2_N);
if Con_2_N <= 0
    Con_2_N = 0;
end
